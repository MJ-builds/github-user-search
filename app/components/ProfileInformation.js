...
export const ProfileInformation = () => {
  const { activeProfile, fetchProfile, profileData } = useGithubContext();

  useEffect(() => {
    fetchProfile(activeProfile);
  }, [activeProfile]);

  //making sure the profileData is first returned before accessing the below properties.
  if (!profileData) {
    return null;
  }
  ...
  //date parsing
  const date = moment(profileData.created_at);
    ...
    <div className=" text-[#4B6A9B] dark:text-[#ffffff] md:p-11 grid grid-cols-1 md:grid-cols-1 p-5 bg-[#FEFEFE] dark:bg-[#1E2A47] rounded-xl shadow-sm md:w-[730px] md:h-[444px]">
      <div className="grid grid-cols-1 md:flex">
        <div className="md:mr-10">
          <img
            className="rounded-full w-[100px] mb-4 md:mb-0"
            src={`${profileData.avatar_url}`}
          />
        </div>
        <div className="flex flex-col flex-grow">
          <div className="flex flex-col md:flex-row justify-between">
            <div className="flex flex-col text-[#2B3442] dark:text-[#ffffff] font-bold text-lg md:text-2xl">
              {profileData.name}
              <div>
                <a
                  className="text-[#0079FF] text-base font-normal text-[11px] md:text-base"
            ...
            <div className="text-[11px] md:text-sm text-[#697C9A] dark:text-[#ffffff] md:mt-2">
              Joined {day} {month} {year}
            </div>
          </div>
          {profileData.bio ? (
            <div className="mt-5 mb-5 text-sm md:text-sm">
              {profileData.bio}
            </div>
          ) : (
            <div className="mt-5 mb-5 text-sm md:text-sm">
          ...
          <div className="bg-[#F6F8FF] dark:bg-[#141D2F] rounded-xl w-auto mt-3">
            <div className="grid grid-flow-col p-5 md:text-sm text-[10px] ">
              <div>
                Repos
                <div className="text-[#2B3442] dark:text-[#ffffff] font-bold text-sm md:text-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 pt-8 gap-4 md:gap-7">
            ...
            <div className="flex flex-row text-sm md:text-md">
              ...
              <a
                className="font-normal hover:underline text-sm md:text-md"
